// wiki-flow — 生成业务流 wiki：flow-wiki-writer 沿 CRG flows 取执行流→按生命周期分组→写页+error_index
// CRG 新鲜度由会话在启动前确认
export const meta = {
  name: 'wiki-flow',
  description: '生成业务流 wiki（调用链+错误目录，给 diag 当直达电梯）',
  whenToUse: '用户要为代码仓生成业务流 wiki。传 args {repo, outDir?, flowPrefix?}。CRG 图须新鲜。',
  phases: [{ title: 'Write', detail: 'flow-wiki-writer 生成业务流页 + error_index' }],
}
...
export default async function ({ agent, phase, log, args }) {
  ...
  const result = await agent(
    ...
    { agentType: 'flow-wiki-writer', label: 'writer' }
  )
