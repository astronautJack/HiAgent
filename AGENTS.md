...
`/feature-design` / `/bug-trace` / `/diag` 返回报告后交主会话呈现人审；批准后才进 implement/fix（未来加）。`/diag` 用「code-tracer 写报告 + 独立 reviewer 审」双 agent loop（最多 3 次），防 code-tracer 自审自圆其说；max loop 未共识则在报告末尾加 `## 存疑点` 段。`subtask:true` 的命令在子会话跑完只返最终报告，中间步骤不污染主上下文。
